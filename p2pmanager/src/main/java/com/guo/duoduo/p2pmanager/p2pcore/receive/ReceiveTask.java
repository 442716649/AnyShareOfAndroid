...
public class ReceiveTask extends Thread {
    ...
    public void run() {
        ...
        for (int i = 0; i < receiver.files.length; i++) {
            ...
            try {
                ...
                Log.d(tag, "prepare to receive file:" + fileInfo.name + "; files size = " + receiver.files.length);
                Log.d("lijun", "prepare to receive file:" + fileInfo.name + "; files size = " + receiver.files.length);
    ...
    private void notifyReceiver(int cmd, Object obj) {
        if (!finished) {
            if (p2PHandler != null)
                p2PHandler.send2Handler(cmd, P2PConstant.Src.RECEIVE_TCP_THREAD, P2PConstant.Recipient.FILE_RECEIVE, obj);
        }
    }
}
