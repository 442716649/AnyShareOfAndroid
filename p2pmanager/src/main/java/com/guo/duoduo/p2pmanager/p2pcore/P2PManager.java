...
public class P2PManager {
    ...
    public void start(P2PNeighbor melon, Melon_Callback melon_callback) {
        ...
        // 开启 CustomHandlerThread子线程，同时为此线程创建一个looper，关联MelonHandler（MelonHandler为CustomHandlerThread子线程的handler）
        p2pThread.start();
        p2pThread.isReady();


        p2PHandler = (MelonHandler) p2pThread.getLooperHandler();
        //开启 MelonCommunicate子线程
    ...
    public void sendFile(P2PNeighbor[] dsts, P2PFileInfo[] files, SendFile_Callback callback) {
        this.sendFile_callback = callback;
        p2PHandler.initSend();

        ParamSendFiles paramSendFiles = new ParamSendFiles(dsts, files);
        p2PHandler.send2Handler(P2PConstant.CommandNum.SEND_FILE_REQ, P2PConstant.Src.MANAGER, P2PConstant.Recipient.FILE_SEND, paramSendFiles);
    }

    public void ackReceive() {
        p2PHandler.send2Handler(P2PConstant.CommandNum.RECEIVE_FILE_ACK, P2PConstant.Src.MANAGER, P2PConstant.Recipient.FILE_RECEIVE, null);
    }
    ...
    public void cancelReceive() {
        p2PHandler.send2Handler(P2PConstant.CommandNum.RECEIVE_ABORT_SELF, P2PConstant.Src.MANAGER, P2PConstant.Recipient.FILE_RECEIVE, null);
    }

    public void cancelSend(P2PNeighbor neighbor) {
        p2PHandler.send2Handler(P2PConstant.CommandNum.SEND_ABORT_SELF, P2PConstant.Src.MANAGER, P2PConstant.Recipient.FILE_SEND, neighbor);
    }

    public static String getSavePath(int type) {
        String[] typeStr = {"APP", "Picture", "Audio", "Video"};
            ...
            switch (msg.what) {
                ...
                case P2PConstant.CommandNum.SEND_FILE_REQ: //收到请求发送文件
                    if (manager.receiveFile_callback != null) {
                        ...
                        manager.receiveFile_callback.QueryReceiving(params.Neighbor, params.Files);
                    }
                    break;
                ...
                case P2PConstant.CommandNum.SEND_PERCENTS:
                    ...
                    if (manager.sendFile_callback != null)
                        manager.sendFile_callback.OnSending((P2PFileInfo) notify.Obj, notify.Neighbor);
                    break;
                ...
                case P2PConstant.CommandNum.SEND_ABORT_SELF: //通知接收者，发送者退出了
                    if (manager.receiveFile_callback != null) {
                        ...
                        if (paramIPMsg != null)
                            manager.receiveFile_callback.AbortReceiving(P2PConstant.CommandNum.SEND_ABORT_SELF, paramIPMsg.peerMSG.senderAlias);
                    }
                    break;
                case P2PConstant.CommandNum.RECEIVE_ABORT_SELF: //通知发送者，接收者退出了
                    if (manager.sendFile_callback != null)
                        manager.sendFile_callback.AbortSending(msg.what, (P2PNeighbor) msg.obj);
                    break;
