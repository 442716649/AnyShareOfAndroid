...
public class MelonCommunicate extends Thread {
    ...
    public void sendMsg2Peer(InetAddress sendTo, int cmd, int recipient, String add) {
        ...
        if (add == null) sigMessage.addition = "null";
        else sigMessage.addition = add;
    ...
    private synchronized void sendUdpData(String sendStr, InetAddress sendTo) {
        try {
            ...
            if (udpSocket != null) {
                ...
                Log.d("lijun", "send upd data = " + sendStr + "; sendto = " + sendTo.getHostAddress());
    ...
    public void run() {
        ...
        while (!isStopped) {
            ...
            try {
                strReceive = new String(resBuffer, 0, receivePacket.getLength(), P2PConstant.FORMAT);
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
                continue;
            }
            ...
            if (!TextUtils.isEmpty(ip)) {
                if (!isLocal(ip)) //自己会收到自己的广播消息，进行过滤
                {
                    if (!isStopped) {
                        ...
                        Log.v("lijun", "sig communicate process received udp message = " + strReceive);
                        ParamIPMsg msg = new ParamIPMsg(strReceive, receivePacket.getAddress(), receivePacket.getPort());
                        p2PHandler.send2Handler(msg.peerMSG.commandNum, P2PConstant.Src.COMMUNICATE, msg.peerMSG.recipient, msg);
                    } else {
                        break;
                    }
                }
    ...
    private void release() {
        ...
        if (receivePacket != null) receivePacket = null;
    }

    private boolean isLocal(String ip) {
        for (int i = 0; i < mLocalIPs.length; i++) {
            if (ip.equals(mLocalIPs[i])) return true;
        }
