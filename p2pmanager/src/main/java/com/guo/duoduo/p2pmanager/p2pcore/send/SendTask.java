...
public class SendTask extends OneByOneRunnable {
    ...
    public void run() {
        ...
        Log.d("lijun", "send task run function");
        super.pause();
    ...
    public int prepare() {
        ...
        try {
            ...
            mappedByteBuffer = fileChannel.map(FileChannel.MapMode.READ_ONLY, socketTransInfo.Offset, socketTransInfo.Length); //将文件映射到内存
    ...
    public void notifySender(int cmd, Object obj) {
        ...
        if (!finished) {
            if (p2PHandler != null)
                p2PHandler.send2Handler(cmd, P2PConstant.Src.SEND_TCP_THREAD, P2PConstant.Recipient.FILE_SEND, notify);
        }
    }
